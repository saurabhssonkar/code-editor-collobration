...
app.post("/register", async (req, res) => {
	const {firstname, user_id, password } = req.body;
	...
	try {
	  ...
	  // Insert into DB
	  await pool.query(
		"INSERT INTO users (user_id, password,firstname) VALUES ($1, $2, $3)",
		[user_id, hashedPassword,firstname]
	  );
  ...
  app.post("/login", async (req, res) => {
	...
	try {
	  ...
	  res.status(500).json({ error: "Login failed", details: "interna error"});
	}
  });
  ...
  app.post('/users/validate', async (req, res) => {
    ...
    try {
		console.log(email)
        const query = 'SELECT 1 FROM users WHERE user_id = $1 LIMIT 1'
